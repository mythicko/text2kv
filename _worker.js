...
export default {
	async fetch (request, env) {
		...
		if (token === mytoken) {
			const file = url.pathname.startsWith('/') ? url.pathname.substring(1) : url.pathname;

			if (file == "config" || file == mytoken) {
			...
			} else if (file == "config/update.bat") {
				return new Response(downloadbat(url.hostname, token), {
				  headers: {
					"Content-Disposition": `attachment; filename=update.bat`, 
					"content-type": "text/plain; charset=utf-8",
				  },
				});
			} else if (file == "config/update.sh") {
				return new Response(downloadsh(url.hostname, token), {
				  headers: {
				...
				if (text === "null" && b64 === "null"){
					...
					await fileExists(KV, file);
					
					// If 'b64' is "null", write the file in plain text; if 'text' is "null", write the file in base64
					if (b64 === "null" ){
						await KV.put(file, text);
						return new Response(text, {
							status: 200,
							headers: { 'content-type': 'text/plain; charset=utf-8' },
						});
					} else if (text === "null" ){
						await KV.put(file, base64Decode(replace(b64)));
						return new Response(base64Decode(replace(b64)), {
							status: 200,
							headers: { 'content-type': 'text/plain; charset=utf-8' },
						});
					}
...
function downloadbat(domain, token) {
	return [
	  `@echo off`,
	  `chcp 65001`,
	  `setlocal`,
...
function downloadsh(domain, token) {
	return `#!/bin/bash
	...
	return `
	  <html>
		...
		<body>
		  ...
		  Windows script: <button type="button" onclick="window.open('https://${domain}/config/update.bat?token=${token}', '_blank')">Click to download</button>
		  <br>
		  ...
		  https://${domain}/<input type="text" name="keyword" placeholder="Please enter the document to be queried">?token=${token}    
		  <button type="button" onclick="window.open('https://${domain}/' + document.querySelector('input[name=keyword]').value + '?token=${token}', '_blank')">View document content</button>
		  <button type="button" onclick="navigator.clipboard.writeText('https://${domain}/' + document.querySelector('input[name=keyword]').value + '?token=${token}')">Copy document address</button>
		  </p>
	  <br>
		</body>
	  </html>
